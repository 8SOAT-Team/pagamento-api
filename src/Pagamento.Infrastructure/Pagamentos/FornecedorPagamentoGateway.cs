...
namespace Pagamento.Infrastructure.Pagamentos
{
        ...
        IPagamentoGateway pagamentoGateway) : IFornecedorPagamentoGateway
    {
        ...
        public async Task<FornecedorCriarPagamentoResponseDto> IniciarPagamento(MetodoDePagamento metodoDePagamento,
            string emailPagador, decimal valorTotal, string referenciaExternaId, Guid pedidoId, string webhookUrl, string token, CancellationToken cancellationToken = default)
        {
            var pedido = await _pagamentoGateway.FindPagamentoByPedidoIdAsync(pedidoId);

            var options = GetRequestOptions(token);
        ...
        public async Task<FornecedorGetPagamentoResponseDto> ObterPagamento(string IdExterno, string token, CancellationToken cancellationToken = default)
        {
            var options = GetRequestOptions(token);
